...
source(paste(r_folder,"MICS_get_data_ReligionX.R",sep=""))
    ...
    for(i in c(1:rn)){
      ...
      datause <- catch_error(get_data(df, rv, dataList, indvar, svnm, educationList, religion_data))
        ...
        if(is.null(datause)) {
          ...
          for(rgv in regList){
            kR<-match(rgv, indvar, nomatch = 0)
            if(kR>0) indvar[kR]<-paste(rgv, "X", sep = "")
            kR<-match(rgv, colnames(datause), nomatch = 0)
            if(kR>0) colnames(datause)[kR]<-paste(rgv, "X", sep = "")
          }
