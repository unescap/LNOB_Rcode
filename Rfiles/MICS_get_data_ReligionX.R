...
# if(length(datause$var2tab[datause$var2tab>=1])>0 & length(datause$var2tab[datause$var2tab==0])>0) {
  ...
  print(indvar)
   for(iv in indvar){
     ...
     else if(iv=="Religion") datause<-Religion(datause, dataList, k, religion_data, svnm)
     else if(iv=="Language") datause<-Language(datause, dataList, k, religion_data, svnm)
...
Religion<-function(datause, dataList, k, religion_data, svnm){
  ...
  datause<-datause[datause$tmp<999,]
  # print(table(datause$tmp))
  if(!(svnm %in% c("Nepal2019", "Vietnam2010",  "VietNam2013"))) datause<-datause[datause$tmp<9,]
  if(length(k)>0 & rk>0){
    if(is.na(k)) return(datause)
    datause$Religion<-"MinorReligion"
    for(i in c(1:rk)){
      print(table(datause$ReligionX))
...
Language<-function(datause, dataList, k, religion_data, svnm){
  ...
  datause<-datause[datause$tmp<999,]
  # print(table(datause$tmp))
  if(!(svnm %in% c("Nepal2019"))) datause<-datause[datause$tmp<9,]
...
add_reglist<-function(data_folder, country_code, version_code, datause, meta_data, dataList, regList, religion_data){
  ...
  dataList<-dataList[!(dataList$DataSet=="hh"),]
  id1<-dataList$VarName[dataList$NickName=="cluster_id"]
  k1<-match(id1, colnames(datause), nomatch = 0)
  id2<-dataList$VarName[dataList$NickName=="HouseholdNumber"]
  k2<-match(id2, colnames(datause))
  if(!is.na(k1) & k1>0) colnames(datause)[c(k1, k2)]<-c("cluster_id", "HouseholdNumber")
