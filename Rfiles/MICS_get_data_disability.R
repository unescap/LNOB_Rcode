...
# if(length(datause$var2tab[datause$var2tab>=1])>0 & length(datause$var2tab[datause$var2tab==0])>0) {
  ...
  # print(indvar)
   for(iv in indvar){
      ...
      if(iv %in% c("Religion", "Ethnicity", "Language")) # & k==0))
     ...
     else if(iv=="Disability") datause<-Disability(datause, dataList, k)   
     else {
...
indList<-function(rv){
  ...
  if(rv %in% c("CleanWater", "HouseholdBasic", "SafeSanitation", "AccessElectricity",
    ...
    return(c("PoorerHousehold", "Residence", "aGroup", "MarriageStatus", "NumberBirths", "Education", "Disability"))
  else if (rv %in% c("ContraceptiveMethod"))
    return(c("PoorerHousehold", "Residence", "aGroup", "NumberBirths", "Education", "Disability"))
    ...
    return(c("PoorerHousehold", "Residence", "aGroup", "NUnder5", "Education", "Disability"))
    ...
    return(c("PoorerHousehold", "Residence", "Education", "Disability"))
  else if (rv %in% c("MobilePhone", "HealthInsurance", "InternetUse", "FinancialInclusion"))
    return(c("PoorerHousehold", "Residence", "aGroup", "Education", "Disability"))
...
Disability<-function(datause, dataList, k){
  ### from wm dataset
  aV<-dataList$VarName[dataList$NickName=="Age"]
  aK<-match(aV, colnames(datause))
  datause$age<-as.numeric(as.character(datause[, aK]))
  datause<-datause[datause$age>=18, ]
  datause$Disability<-0
  ...
  for(i in k){
    if(!is.na(i)){
      ...
      datause$Disability[datause[, i] %in% c(3, 4)]<-1
    }
  }
  
  print("Disability proportion is")
  print(sum(datause$Disability*datause$SampleWeight)/sum(datause$SampleWeight)*100)
  return(datause)
