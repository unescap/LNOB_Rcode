...
output_data<-function(datause, country_code, version_code, rv, rtp, indvar, ds, ds_output_folder, validationdata, 
                      religion, region_flag, use_version, drupalIndex){

  print(region_flag)
  #### to validate if this is needed
  result_log_National<-ResultList(country_code, version_code, ds, rv, religion, "National", !is.null(datause))
  ...
  if(use_version==1 | validation){
    ...
    for(rg in regionList){
      ...
      t0<-drupalIndex
      tree_result<-write_tree(datauseRG, country_code, version_code, rg, 
      ...
      d_result<-write_HOI_D(datauseRG, country_code, version_code, rg, rv, ds, title_string, indvar, 
...
write_tree <- function(datause, country_code, version_code, region, 
  ...
  if(use_version==3 & !is.null(tree_stat$tree_stat))
  {
    ...
    if(region=="National") drupal_data<-dataForDrupal(tree_stat[2], "tree_data", "MICS", ds, paste(country_code, version_code, rv, sep=" - "), formula_string, 
                                                      country_code, version_code, rv, region)
    else drupal_data<-dataForDrupal(tree_stat[2], "region_tree_data", "MICS", ds, paste(country_code, version_code, rv, sep=" - "), formula_string, 
                                    country_code, version_code, rv, region)
...
write_HOI_D <- function(datause, country_code, version_code, region, rv, ds, title_string,
  ...
  result<-catch_error_prod(cal_HOI_shapley(datause, indvar))
  if(use_version==3 & !is.null(result))
  {
    ### for version 3, we store the data in one folder for publication
    if(region=="National") drupal_data<-dataForDrupal(result$drupalData, "d_index", "MICS", ds, paste(country_code, version_code, rv, sep=" - "), formula_string, 
                               country_code, version_code, rv, region)
    
    else drupal_data<-dataForDrupal(result$drupalData, "region_d_index", "MICS", ds, paste(country_code, version_code, rv, sep=" - "), formula_string, 
                                    country_code, version_code, rv, region)
...
ResultList<-function(country_ISO, version_code, ds, rv, religion, region, isNullData){
  
  Result<-list(
    country=country_ISO,
    countryName=isoTOcountry(country_ISO),
    year=version_code, 
    dataSource="MICS",
    dataSet=ds,
    indicator=rv,
